...
SCENARIO("Testing SkipList class deletion") {
	srand(time(NULL));
...
SCENARIO("Test SkipList class operators and constructors")
{
	...
	GIVEN("Test move constr and move operator=") {
		...
		WHEN("Fill the default slist with 1000 random values") {
			...
			THEN("Move with constr") {
				SkipList movedslist = std::move(slist);
